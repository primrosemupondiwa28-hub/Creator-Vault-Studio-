  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-luxury-950/90 backdrop-blur-md p-4">
      <div className="w-full max-w-md bg-luxury-900 border border-brand-900/50 rounded-2xl shadow-2xl p-8 relative overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 text-brand-400/50 hover:text-brand-200 transition-colors p-2 hover:bg-brand-900/20 rounded-full"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-brand-300/60 text-sm">
            To generate images and enter this room, please provide your Google Gemini API Key.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="relative">
              <input 
                ...
                placeholder="AIzaSy..."
                className="w-full bg-luxury-950 border border-brand-900 rounded-xl px-4 py-3 pl-11 text-brand-50 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none transition-all placeholder:text-brand-900"
              />
          ...
          <button 
            onClick={handleSave}
            className="w-full py-3 bg-gradient-to-r from-brand-600 to-brand-500 hover:to-brand-400 text-white font-bold rounded-xl shadow-lg shadow-brand-900/20 transition-all flex items-center justify-center gap-2"
          >
            Enter Studio
          </button>
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-brand-900/30 text-center">
          <p className="text-xs text-brand-400/50 mb-2">Don't have a key?</p>
          <a 
            href="https://aistudio.google.com/app/apikey" 
            target="_blank" 
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1 text-xs text-brand-400 hover:text-brand-200 transition-colors border-b border-brand-500/30 pb-0.5 hover:border-brand-200"
          >
            Get a free API Key from Google <ExternalLink className="w-3 h-3" />
          </a>
        </div>
        
        <div className="mt-4 flex items-center justify-center gap-2 text-[10px] text-emerald-500/60 bg-emerald-950/20 py-2 rounded-lg">
